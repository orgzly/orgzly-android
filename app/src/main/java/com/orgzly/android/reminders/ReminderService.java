...
public class ReminderService extends IntentService {
    ...
    private static boolean isRelevantNoteTime(Context context, TimesClient.NoteTime noteTime) {
        ...
        boolean scheduled = AppPreferences.remindersForScheduledEnabled(context) && noteTime.timeType == TimesView.SCHEDULED_TIME;
        boolean deadline = AppPreferences.remindersForDeadlineEnabled(context) && noteTime.timeType == TimesView.DEADLINE_TIME;
    ...
    private static ReadableInstant[] getInterval(int beforeOrAfter, ReadableInstant now, LastRun lastRun, int timeType) {
        ...
        switch (beforeOrAfter) {
            case TIME_BEFORE_NOW: // Before now, starting from lastRun, depending on timeType
                res[0] = timeType == TimesView.SCHEDULED_TIME ? lastRun.scheduled : lastRun.deadline;
                if (res[0] == null) {
                    res[0] = now;
                }
                res[1] = now;
    ...
    private void showNotification(Context context, List<NoteReminder> notes) {
        ...
        for (int i = 0; i < notes.size(); i++) {
            ...
            String line = context.getString(
                    noteReminder.getPayload().timeType == TimesView.SCHEDULED_TIME ? R.string.reminder_for_scheduled : R.string.reminder_for_deadline,
